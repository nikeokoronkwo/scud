...
Args get_args(std::vector<std::unique_ptr<scud::ast::ParameterExpr>>& exprs) {
  ...
  for (int i = 0; i < exprs.size(); ++i) {
    ...
    std::string index =
        val->named
            ? static_cast<scud::ast::IdentifierExpr*>(val->name.release())->name
            : std::to_string(i);
    switch (val->value.get()->kind) {
      ...
      case scud::ast::ExprKind::List: {
        ...
        // std::transform(b->values.cbegin(), b->values.cend(),
        // std::back_inserter(listvals), [](const
        // std::unique_ptr<scud::ast::Expr>& ptr){
        //   return *(ptr.get());
        // });
