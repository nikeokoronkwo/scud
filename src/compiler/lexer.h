...
class Token {
 public:
  // Enum to represent the given types of tokens, with the underlying type of
  // `unsigned int`
  enum TokenKind {
    unknown = 0,
    eof,
 ...
 public:
  ...
  Token(std::string m_lexeme, TokenKind m_kind, int m_line = 0, int m_char = 0)
      : lexeme(m_lexeme), kind(m_kind), line(m_line), character(m_char) {}

  // The token as a debug string
  std::string to_string() const;
};

// Representation class of a lexer to scan and parse the given syntax from a
// source string
//
