...
namespace ast {
    ...
    void StringExpr::accept(Visitor& visitor) { return visitor.visit_string_expr(*this); }
    void ListExpr::accept(Visitor& visitor) { return visitor.visit_list_expr(*this); }
    void IdentifierExpr::accept(Visitor& visitor) { return visitor.visit_identifier_expr(*this); }
    void VariableExpr::accept(Visitor& visitor) { return visitor.visit_variable_expr(*this); }
    void ParameterExpr::accept(Visitor& visitor) { return visitor.visit_parameter_expr(*this); }
    void FunctionExpr::accept(Visitor& visitor) { return visitor.visit_function_expr(*this); }
...
std::unique_ptr<ast::Expr> Parser::parse_list_expr() {
    ...
    while (current().kind != Token::TokenKind::right_square_paren) {
        if (current().kind == Token::TokenKind::eof) log_error("Expected ']', got to end of file");
        
        auto v = parse_expr();
        if (!v) expressions.push_back(nullptr);

        if (current().kind == Token::TokenKind::right_square_paren) { break; }

        if (current().kind != Token::TokenKind::comma) { 
            log_error(("Expected ',' in list, but got " + current().to_string()).c_str()); 
        } else {
            next(); // eat ','
        }
    }
...
std::unique_ptr<ast::Expr> Parser::parse_identifier_expr() {
    ...
    if (current().kind != Token::TokenKind::right_paren) {
        ...
        while (true) {
            ...
            if (arg != nullptr) args.push_back(std::move(
                std::unique_ptr<ast::ParameterExpr>(static_cast<ast::ParameterExpr *>(arg.release()))
            )); 
            else return nullptr;
            ...
            if (current().kind != Token::TokenKind::comma) return log_error("Expected ')' or ',' in argument call for function.");
            next();
        }
...
std::unique_ptr<ast::Expr> Parser::parse_param_expr() {
    ...
    if (current().kind == Token::TokenKind::colon) {
        if (curr.kind != Token::TokenKind::identifier) return log_error("Expected identifier before colon in named parameter");
        next(); // eat ':'
...
std::unique_ptr<ast::Expr> Parser::parse_expr() {
    ...
    default:
        return log_error(("unknown token when parsing expression: " + current().to_string()).c_str());
        break;
    }
}
    ...
    while (loop) {
        switch (current().kind) {
            ...
            default:
                ...
                if (expr != nullptr) expressions.push_back(std::move(expr));
                else exit(0);
                break;
        }
    }
