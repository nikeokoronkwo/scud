...
namespace scud {
// Simple method to convert a `TokenKind` to a representable string, for
// debugging purposes
std::string Token::stringify_token_kind(TokenKind kind) const {
  switch (kind) {
    ...
    case left_square_paren:
      return "left square parenthesis";
      break;
    case right_square_paren:
      return "right square parenthesis";
      break;
    case right_paren:
...
std::string Token::to_string() const {
  return lexeme + ": " + stringify_token_kind(kind) + " at line " +
         std::to_string(line) + " character " + std::to_string(character);
}

// Function to proceed to the next token
const char Lexer::next() { return source[++ptr]; }

// Whether the lexer has reached the end of the file
bool Lexer::at_end() const { return ptr == source.size() - 1; }

// Adds a token to the list
void Lexer::add_token(const char* c, Token::TokenKind kind) {
  tokens.push_back(Token(c, kind, line, ptr + 2));
}
void Lexer::add_token(std::string c, Token::TokenKind kind) {
  tokens.push_back(Token(c, kind, line, ptr + 2));
}
...
void Lexer::scan_token() {
  ...
  switch (c) {
    case ')':
      add_token(")", Token::TokenKind::right_paren);
      break;
    case '(':
      add_token("(", Token::TokenKind::left_paren);
      break;
    case ':':
      add_token(":", Token::TokenKind::colon);
      break;
    case ']':
      add_token("]", Token::TokenKind::right_square_paren);
      break;
    case '[':
      add_token("[", Token::TokenKind::left_square_paren);
      break;
    case '=':
      add_token("=", Token::TokenKind::equals);
      break;
    case '{':
      add_token("{", Token::TokenKind::left_brace);
      break;
    case '}':
      add_token("}", Token::TokenKind::right_brace);
      break;
    case ',':
      add_token(",", Token::TokenKind::comma);
      break;
    case '\n':
      line++;
      break;
