...
std::string build_command_from_exe(ScudExecutable exe) {
  std::string compiler = internal::set_compiler_from_lang(exe.lang, exe.defines);

  switch (exe.lang) {
    case c:
    case cpp: {
      std::vector<std::string> includes;
      std::transform(exe.include.cbegin(), exe.include.cend(), std::back_inserter(includes), [](ScudInclude& include){ return include.path; });
...
std::string make_executable(ScudExecutable exe) {
  ...
  std::string target = exe.name + internal::exe_extension(shared::currentPlatform());
